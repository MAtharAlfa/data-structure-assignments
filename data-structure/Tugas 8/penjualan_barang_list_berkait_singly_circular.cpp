...
Deskripsi    : Program akan mengelola data penjualan barang berikut ➔ Barang (kodeBrg, namaBrg, 
               ...
               struktur data singly circular linked list
*/
...
Pointer getLast(List first) {
    ...
    } else
    {
        ...
        {
            pHelp = pHelp->next;
        } while (pHelp->next != first);

        return pHelp;
    }
}

Pointer getPrecNode(List first, Pointer pTarget) {
    if (!first)
    {
        std::cerr << "\nList kosong!\n";
        return nullptr;
...
void destroyList(List& first) {
    ...
    Pointer pHelp = first, last = getLast(first), pHapus = nullptr;
...
Pointer deleteBefore(List& first, Pointer& pSearch) {
    Pointer pHapus = nullptr, precSearch = getPrecNode(first, pSearch); 
    ...
    {
        pHapus = precSearch;
        Pointer precHapus = getPrecNode(first, pHapus);
        
        precHapus->next = pSearch;
...
void insertBefore(List& first, Pointer& pSearch , Pointer& pNew) {
    ...
    {    
        Pointer precSearch = getPrecNode(first, pSearch);
        pNew->next = pSearch;
        precSearch->next = pNew;
    }
}
...
void insertFirst(List& first, Pointer& pBaru) {
    ...
    {
        Pointer last = getLast(first);

        pBaru->next = last->next;
        last->next = pBaru;
...
Pointer deleteLast(List& first) {
    ...
    {
        Pointer last = getLast(first), precLast = getPrecNode(first, last);
